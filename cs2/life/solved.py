...
for line in lines:
    ...
    row = [sym != "." for sym in line]
    cells.append(row)
    neighbors.append([0] * width)


def showCells(cells):
    return "\n".join("".join("██" if cell else "░░" for cell in row) for row in cells)
    ...
    for r in range(height):
        for c in range(width):
            ...
            for dr in [-1, 0, 1]:
                for dc in [-1, 0, 1]:
                    if dr or dc:
                        neighbors[r][c] += cells[(r + dr) % height][(c + dc) % width]

    for r in range(height):
        for c in range(width):
            cells[r][c] = neighbors[r][c] == 3 or cells[r][c] and neighbors[r][c] == 2
